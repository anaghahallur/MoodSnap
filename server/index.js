...
const authenticateUser = async (req, res, next) => {
    ...
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
        return res.status(401).json({ message: 'Authentication required', raw: 'No valid Bearer token found in headers' });
    }
    ...
    try {
        ...
        if (error || !user) {
            console.error('Supabase raw auth error:', error);
            return res.status(401).json({
                message: 'Invalid or expired token',
                supabase_error: error ? error.message : 'User data is null',
                token_snippet: token.substring(0, 10) + '...'
            });
        }
        ...
        res.status(500).json({ message: 'Authentication failed severely', internal_error: error.message });
    }
};
